...
var rpt = true;
var zooming = 16;
var mapsUrl = '/Maps/GetSitePerFilter?';
var markers = [];
var placingEx = null;
var infoWindow;
var AllQueriedSites = [];
...
function SetExtraInfo() {
	debugger
	$.get("/Maps/GetCountryInfo?name=" + rCountry, function (data) {
		if (data.length > 0) {
			var cntr = 0;
	...
	if (!IsNullOrEmptyNew(rCountry)) {
		...
		geocoder.geocode({ 'address': rCountry }, function (results, status) {
			if (status === window.google.maps.GeocoderStatus.OK) {
				myLatLng = {
					lat: results[4].geometry.location.lat(),
					lng: results[5].geometry.location.lng()
				};
				DrawMap2();
			} else
...
function GetSitePerFilter2(param) {
	debugger
	$.ajax({
		url: mapsUrl + "param=" + param + "&filter=" + id + "&lat=" + myLatLng.lat + "&lng=" + myLatLng.lng,
		type: 'GET',
		dataType: 'json',
		contentType: 'application/json; charset=utf-8',
		error: function (xhr) {
			alert("Error en la ejecución, por favor recargue la página getsiteperfilter2" + xhr);
		},
		success: function (result) {
			if (result.length < 1) {
...
function DrawSucursal(i, j) {
	...
	var latLng = { lat: parseFloat(j.latitude), lng: parseFloat(j.longitude) };
	console.log(latLng)
	...
	if (type === "prestador") {
		contentString = '<div id="iw-container" >' +
            '<span class="iw-title">' + j.placeholder + '</span>' +
            '<div class="gm-style-iw gm-style-iw-body">' +
            '<label style="display:block;"><span class="bolding">' + j.placeholder + ':</span> <span class="lighting">' + j.placeholder + '</span></label>' +
            '<label style="display:block;"><span class="bolding">' + j.address + ':</span> <span class="lighting"> ' + j.address + '</span></label>' +
            '<label style="display:block;"><span class="bolding">' + j.phone + ':</span> <span class="lighting">' + j.phone + '</span></label>' +
            '<label style="display:block;"><span class="bolding">' + j.email + ':</span> <span class="lighting"> ' + j.email + '</span></label>' +
            '</div>' +
            '</div>';
		...
		leftWriting = '<div class="col-md-12 sucursal"  tabindex="' + i + '" id="' + j.Id + '" data-country="' + j.Country + '" data-city="' + j.City + '" data-name="' + j.Name + '" id="SucursalId' + i + '" >' +
            '<h3 class="sucursal-title">' + j.placeholder  + '</h3>' +
            '<h4 class="sucursal-item"><span class="bolding">' + j.placeholder + ':</span> <span class="lighting">' + j.placeholder + '</span></h4>' +
            '<h4 class="sucursal-item"><span class="bolding">' + j.address + ':</span> <span class="lighting">' + j.address + '</span></h4>' +
            '<h4 class="sucursal-item"><span class="bolding">' + j.phone + ':</span> <span class="lighting">' + j.phone + '</span></h4>' +
            '<h4 class="sucursal-item"><span class="bolding">' + j.email + ':</span> <span class="lighting">' + j.email + '</span></h4>' +
            specs +
            '</div>';
		...
		contentString = '<div id="iw-container">' +
            ...
            '</div>' +
            '</div>';
		leftWriting = '<div class="col-md-12 sucursal"  tabindex="' + i + '" id="' + j.Id + '" data-country="' + j.Country + '" data-city="' + j.City + '" data-name="' + j.placeholder + '" >' +
            '<h3 class="sucursal-title">' + j.placeholder + '</h3>' +
            '<h4 class="sucursal-item"><span class="bolding">' + j.address + ':</span> <span class="lighting">' + j.address + '</span></h4>' +
            '</div>';
		$('#Sucursales').append(leftWriting);
